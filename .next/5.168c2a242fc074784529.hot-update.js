...
function (_React$Component) {
  ...
  _createClass(Image, [{
    ...
    value: function pushStateProject(id, slug, nextId, nextSlug) {
      console.log(id, slug, nextId, nextSlug); // if(this.props.isImage==true){
      //     Router.pushRoute(`/anh/${id}-${slug}`,`/anh/${nextId}-${nextSlug}`)
      // }else{
      //     Router.push(`${this.props.path}?photoId=${id}&id=${this.state.project.id}`,`/anh/${nextId}-${nextSlug}`)
      // }
    }
  }, {
    key: "render",
...
//# sourceMappingURL=5.168c2a242fc074784529.hot-update.js.map