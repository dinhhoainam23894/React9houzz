        ...
        <div id="image-container">
          <div className="image">
            {
              ...
              <LazyLoad once placeholder={<Placeholder/>} debounce={0}>
                <img className="image-detail" src={this.state.currentValue.path_for_size}
          ...
          <div id="lbActions" className="d-none d-md-block">
            <div id="lbActionCenter" className="offset-0 offset-md-3 col-12 col-md-6 text-center text-nowrap">
              <button className="btn btn-primary med save text-white mr-3" title="Save To Ideabook"
                      compid="addToIdeabook"><i className="fa fa-plus pr-2"></i>Lưu ảnh
              </button>
              <button className="btn bg-black-100 med email text-white" title="send email" compid="addToIdeabook"><i
                className="fa fa-envelope-o pr-2"></i>Gửi Email
              </button>
            </div>
          </div>
        </div>
            ...
            <nav>
              <div className="nav nav-tabs" id="nav-tab" role="tablist">
                <a className="nav-item nav-link active" id="nav-home-tab" data-toggle="tab" href="#nav-home" role="tab"
                   aria-controls="nav-home" aria-selected="true"><i className="fa fa-home"></i></a>
                <a className="nav-item nav-link" id="nav-profile-tab" data-toggle="tab" href="#nav-profile" role="tab"
                   aria-controls="nav-profile" aria-selected="false"><i className="fa fa-tag"></i></a>
                <a className="nav-item nav-link" id="nav-contact-tab" data-toggle="tab" href="#nav-contact" role="tab"
                   aria-controls="nav-contact" aria-selected="false"><i className="fa fa-comment"></i></a>
              </div>
            </nav>
          </div>
        </div>
          ...
          <div className="content-scroll">
                  ...
                  <div className="media-content">
                    {
                      ...
                      <Link route={`/pro/${provider.id}-${provider.slug}`}>
                        <a
                          className="font-weight-bold font-14 text-black-100">{provider ? provider.name : 'Chưa có tên'}</a>
                      </Link>
                    }
                    <div className="star-rating font-14">
                      <span
                        className="text-black-100 font-14">{provider && rating(provider.avg_rate)}{provider ? "(" + provider.total_rate + " người đánh giá" + ")" : "(0 người đánh giá)"}</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="content-detail border-0">
              <ol className="breadcrumb bg-white pl-0 mb-0 pt-0 mt-0">
                ...
                {tag.breadcrumbs &&
                <li className="breadcrumb-item" itemScope itemType="http://data-vocabulary.org/Breadcrumb">
                  <Link route={tag.breadcrumbs.uri}><a itemProp="url"><span itemProp="title"
                                                                            className="font-13">{tag.breadcrumbs.name_tag}</span></a></Link>
                </li>}
              </ol>
              ...
              <div className="media-content" id="readMore">
                <div className="readMoreWrapper">
                  {currentValue &&
                  <p id="readMoreText" className="font-14 normalText"
                     dangerouslySetInnerHTML={{__html: currentValue.descriptions}}/>
                  }
            ...
            <div className="content-detail border-0">
              ...
              <ul className="list-unstyled clearfix thumb-grid grid-5">
                {
                  images && images.map((value, index) => (
                    <li className="thumb project-thumb" data-id={value.id} ref="'image'+image.id" data-slug={value.slug}
                        key={index}>
                      <a className="link" href={`/anh/${value.id}-${value.slug}`}
                         onClick={(e) => this.changeImage(e, value)}>
                        <div className="img-responsive-wrapper img-responsive-square">
                          {
                            value.small_path &&
                            <LazyLoad
                              placeholder={<Placeholder dataSrc={value.small_path}/>}>
              ...
              <div className="pt-0">
                {
                  tag.breadcrumbs &&
                  <Link route={tag.breadcrumbs.uri}>
            ...
            <div className="content-detail border-0">
              <div className="header row m-0">
                ...
                <span className="col-xs-12 col-md-12 px-0"><button id="askQuestionButton"
                                                                   className="btn border-primary btn-block text-primary font-13"
                                                                   compid="lbAsk">Đặt câu hỏi của bạn</button></span>
              </div>
            </div>
          </div>
        </div>
